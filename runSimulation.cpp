...
int slice_chunk(const unsigned nprocs, const int Slice[][DIMENSION][2], int *xs, int *xe, int *ys, int *ye, \
					  ...
					  int N_PROCS, int Slice[][DIMENSION][2]) 
{
 		...
 		for(unsigned nprocs=0; nprocs<N_PROCS; nprocs++)
		{
        	...
        	auto v = graph.addVertex(Streaming,
                                ...
                                {{"nNodeType",  	NodeType.slice({ys,zs,xs},{ye,ze,xe}).flatten()         	},
                                 ...
                                 {"Move",        	Move.slice({nprocs,ds},{nprocs+1,de}).flatten() 			},
                                 {"nOppIndex",      OppIndex                               						},
                                 {"nx",     		chunk_nx                               						},
                                 {"ny",     		chunk_ny                                            		},
                                 {"nz",     		chunk_nz                                            		},
    ...
    ComputeSet BoundaryCavity = graph.addComputeSet("BoundaryCavity");
    {
		...
		{
			if(slice_chunk(nprocs,Slice,&xs,&xe,&ys,&ye,&zs,&ze,&chunk_nx,&chunk_ny,&chunk_nz,&chunk_len)) continue;
        	//printf("TILE=%4d | %02d-%02d | %02d-%02d | %02d-%02d | %2d %2d %2d\n",nprocs, xs,xe,ys,ye,zs,ze,chunk_nx,chunk_ny,chunk_nz);
